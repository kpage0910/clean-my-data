...
This module powers the "Quick Apply" workflow—automatic cleaning that requires
minimal user input. It infers column types and applies appropriate transformations.

HOW IT WORKS:
─────────────
1. TYPE INFERENCE: Analyze each column to guess what kind of data it contains
   (name, email, date, currency, etc.) based on column names and sample values.
   
2. RULE GENERATION: Based on inferred types, generate appropriate cleaning rules.
   For example: email columns get lowercased, name columns get title-cased.
   
3. TRANSFORMATION: Apply the generated rules using the cleaner service.

WHY "AUTONOMOUS"?
─────────────────
Unlike the "Safe Review" workflow (suggestion_engine.py), this engine makes
decisions automatically. It's designed for users who want quick results and
trust the system to make reasonable choices.

STRICT MODE STILL APPLIES:
──────────────────────────
Even in autonomous mode, we follow strict mode rules by default:
...
We still NEVER:
✗ Guess or fabricate missing data
✗ Impute values without explicit permission
✗ Change the semantic meaning of any value

COLUMN TYPE INFERENCE:
──────────────────────
The inference system uses two signals:
1. Column name patterns (e.g., "email" → EMAIL type, "price" → CURRENCY type)
2. Value patterns (e.g., values with @ → EMAIL, values with $ → CURRENCY)

See InferredType enum for all supported types.
"""
...
# ============================================
# The core of autonomous cleaning: figuring out what each column contains.

class InferredType(str, Enum):
    """
    Semantic column types that we can detect and clean appropriately.
    
    Each type has specific cleaning rules:
    - NAME: Title case, trim whitespace
    - EMAIL: Lowercase, validate format
    - DATE: Standardize to ISO format
    - NUMERIC: Convert number words, remove formatting
    - etc.
    """
    NAME = "name"
    EMAIL = "email"
...
class ColumnInference(BaseModel):
    """
    Result of inferring a column's semantic type.
    
    The inference includes a confidence score (0-1) so downstream code
    can decide whether to trust it. Low confidence means the inference
    might be wrong—proceed with caution.
    """
...
class GeneratedRule(BaseModel):
    """
    A cleaning rule auto-generated by the autonomous engine.
    
    Rules are sorted by priority (lower = earlier) so they execute in
    the right order. For example, whitespace trimming should happen
    before capitalization normalization.
    """
...
# ============================================
# These regex patterns help us identify column types from names and values.
# They're intentionally permissive to catch variations (e.g., "email", "e-mail", "Email").

# --- Name columns ---
# Matches: name, first_name, lastName, customer_name, etc.
...
# --- Numeric/Currency/Percentage columns ---
...
# --- ID columns (usually should NOT be modified) ---
...
# Used for converting "thirty" → 30, "one hundred" → 100, etc.
NUMBER_WORDS = {
...
# Import compound number phrase parser (handles "twenty five" → 25)
...
def infer_column_type(df: pd.DataFrame, column: str) -> ColumnInference:
    ...
    The inference uses a priority system:
    1. First check column name (fast, often accurate)
    2. Then check value patterns (slower, but catches unnamed columns)
    
    Returns a ColumnInference with:
    - inferred_type: What we think the column contains
    - confidence: How sure we are (0-1)
    - indicators: Evidence supporting our inference (for debugging/UI)
    - is_safe: Whether it's safe to auto-clean this column
    """
    col_name = column
